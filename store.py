...
try:
    conn = sqlite3.connect("Final Project CSA\store.db")
    c = conn.cursor()

    # Create a table
    c.execute('''CREATE TABLE IF NOT EXISTS products (name TEXT, stock INTEGER, cp REAL, sp REAL, vendor TEXT, vendor_phone TEXT)''')
    conn.commit()
...
frame4 = tkinter.Frame(root, bg="#51A3A3")
...
def ajax():
    ...
    pprice.configure(text="Price: $. " + str(get_price))
...
def add_to_cart():
    ...
    if quantity_value > int(get_stock):
        tkinter.messagebox.showinfo(
            "Error", "Ahh So sorry! Not that many products in our inventory:)).")
    else:
        # calculate the price
        ...
        for p in products_list:
            ...
            # total configure
            total_l.configure(
                text="Total: $." + str(sum(product_price)))
...
# frame1
left = Frame(frame1, width=700, height=768, bg='white')
left.pack(side=LEFT)

right = Frame(frame1, width=666, height=768, bg='Teal')
right.pack(side=RIGHT)

# components
heading = Label(left, text="24h Mart", font=(
    'arial 40 bold'), bg='floral white')
heading.place(x=0, y=0)

date_label = Label(right, text="Today's Date: " +
                   str(date), font=('arial 16 bold'), bg='teal', fg='white')
...
productname = Label(left, text="", font=(
    'arial 16 bold'), bg='floral white', fg='steelblue')
productname.place(x=0, y=190)

pprice = Label(left, text="", font=(
    'arial 16 bold'), bg='floral white', fg='steelblue')
pprice.place(x=0, y=160)
...
add_to_cart_btn.place(x=350, y=350)
...
home_btn = Button(frame2, text="Home",
                  height=2, bg='teal', fg='white', command=lambda: show_frame(frame1))
home_btn.place(x=30, y=690)


update_btn = Button(frame2, text="Update products",
                    height=2, bg='teal', fg='white', command=lambda: show_frame(frame3))
update_btn.place(x=150, y=690)

logout_btn2 = Button(frame2, text="Logout",
                     height=2, bg='teal', fg='white', command=lambda: show_frame(frame4))
...
canvas.create_rectangle(0, 0, 725, 700, fill='#F5F5F5', outline="black")

heading = Label(frame4, text="24h Mart",
                font=('Helvetica 48 bold'), fg='White', bg="teal")
...
Dashes = Label(frame4, text="--------------------------------------",
               font=('helvetica 40 bold'), fg='white', bg="teal")
Dashes.place(x=350, y=225)

Uname_l = Label(frame4, text="Username :", font=(
    'arial 18 bold'), fg='white', bg="teal")
